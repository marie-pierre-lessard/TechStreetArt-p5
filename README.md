...
The web app as a whole constitutes a styled art viewer with a responsive image carousel/selector that accommodates a broad range of pictures (regardless of their orientation and width-to-height ratio), a reset button, a button to save and download the user's art, some text content, and an art game. Everything outside of the enhanced kaleidoscope function was meant to be reused as the setting and components of other games. The enhanced kaleidoscope function is a derivative work of the following P5 example, which I modified in order to create a kaleidoscope that was complete with a colour picker. 
...
### Notes
* The pictures are no longer the class project's pictures, which had different orientations, but the same number of pixels. The links to these pictures are broken.
* I cannot fully explain why randomly picked Unsplash pictures all get displayed with the landscape orientation, which leads portrait images to be distorted instead of being displayed vertically. This doesn't happen with randomly picked paths to specific image files in an array. The cause is probably related to the fact that the Unsplash endpoints are currently processed by an [API by Imgix](https://unsplash.com/blog/the-service-powering-unsplashs-beautiful-imagery/), which intuitively transforms images.
* If I had used the `resize()` method of the `p5.Image` object instead of solving the problem mathematically and setting both a width and a height, there may not have been a problem with Unsplash (`resize()` corresponds to using the aspect-ratio CSS property). However, it is unlikely that I will update this assignment's code to make it work with Unsplash/Imgix since this wasn't the goal of the original exercise. Instead, I might replace the Unsplash endpoints by file paths.
* The format of Unsplash pictures had to be kept manageable because there is a limit to how big the pictures can be without causing side-scroll in the art viewer on small devices.
...
The images are picked randomly by [Unsplash](https://unsplash.com), which has a very permissive [open-source license](https://unsplash.com/license). This license applies to any image modified with the art game. 
