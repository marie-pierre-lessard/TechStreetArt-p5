...
(Please note that the pictures are no longer the class project's pictures, which had different orientations, but the same number of pixels. The links to these pictures are broken. At this time, I cannot explain why randomly picked Unsplash pictures all get displayed with the landscape orientation, which leads portrait images to be distorted instead of being displayed vertically. This doesn't happen with randomly picked paths to specific image files in an array. Also note that there is a limit to how big the pictures can be without causing side-scroll in the art viewer on small devices, so the format of Unsplash pictures had to be kept manageable.)
