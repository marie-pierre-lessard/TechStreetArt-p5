...
The pictures are no longer the class project's pictures, which had different orientations, but the same number of pixels. The new images are picked randomly by [Unsplash](https://unsplash.com), which has a very permissive [open-source license](https://unsplash.com/license). This license applies to any image modified with the art game. (Please note that I cannot explain why randomly picked Unsplash pictures all tend to have the same orientation: portrait or landscape. This undermines the quality of the demonstration. This doesn't happen with randomly picked paths to image files in an array.)
